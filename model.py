...
def build_enc_dec(source,reuse=False) :
    ...
    batch_size, _, image_size, channels = source.get_shape().as_list()
    ...
    with tf.variable_scope('decoder') as s:
        ...
        for l,(in_,out_,size_) in enumerate([(128,64,image_size//2),(64,32,image_size)]):
            decoder_spec += [
                TransposedConv2d('tconv_%d'%(l+1),in_,[batch_size,size_,size_,out_],4,4,2,2),
...
def build_critic(_t) :
    _, _, image_size, channels = _t.get_shape().as_list()
