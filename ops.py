...
class TransposedConv2d(object):
    def __init__(self,name,input_dim,output_shape,
        ...
        with tf.variable_scope(name) :
            self.w = tf.get_variable('w', [k_h, k_w, output_shape[3], input_dim],
                                initializer=tf.random_normal_initializer(stddev=stddev))
            self.b = tf.get_variable('b',[output_shape[3]], initializer=tf.constant_initializer(0.0))
...
class InstanceNorm():
    def __init__(self,name,format='NHWC',epsilon=1e-5) :
